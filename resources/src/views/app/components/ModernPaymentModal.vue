                        ...
                        <div class="credit-card-section">
                          <div class="cc-header">
                            ...
                            <div class="cc-actions">
                              <button
                                v-if="(localSavedPaymentMethods && localSavedPaymentMethods.length) && !isSavedMode(idx)"
                                type="button"
                                class="action-btn cancel-btn"
                                @click="show_saved_credit_card(idx)"
                              >
                                <i class="i-Two-Windows"></i> {{$t('Use_Saved_Credit_Card')}}
                              </button>
                              <button
                                v-if="isSavedMode(idx)"
                                type="button"
                                class="action-btn cancel-btn"
                                @click="show_new_credit_card(idx)"
                              >
                                <i class="i-Add"></i> {{$t('Enter_New_Credit_Card')}}
                              </button>
                            </div>
                          </div>

                          <div v-if="isLineLoadingSavedCards(idx)" class="mt-2">
                            <div class="spinner sm spinner-primary"></div>
                          </div>

                          <!-- Saved Cards List -->
                          <div v-if="isSavedMode(idx) && !isLineLoadingSavedCards(idx)">
                            <div class="saved-cards">
                              <div class="saved-cards-header">{{$t('Saved_Credit_Cards')}}</div>
                              <table class="table table-sm saved-cards-table">
                                <thead>
                                  <tr>
                                    <th>{{$t('Brand')}}</th>
                                    <th>{{$t('Last4')}}</th>
                                ...
                                <tbody>
                                  <tr v-for="card in localSavedPaymentMethods" :key="card.card_id" :class="{ 'bg-selected-card': isSelectedCardForLine(card, idx) }">
                                    <td>
                                      {{ card.brand || card.card_brand || $t('Card') }}
                                      <span v-if="localSavedPaymentMethods.length > 1 && String(card_id) === String(card.card_id)" class="default-badge">{{$t('Default')}}</span>
                                      <span v-if="isSelectedCardForLine(card, idx)" class="selected-badge">{{$t('Selected')}}</span>
                                    </td>
                                    <td>{{ card.last4 || card.card_last4 || '****' }}</td>
                                    <td>{{ formatCardExpiry(card) }}</td>
                                    <td class="text-right">
                                      <template v-if="!isSelectedCardForLine(card, idx)">
                                        <b-button size="sm" variant="outline-primary" @click="selectCardForLine(card, idx)">
                                          <i class="i-Yes"></i> {{$t('Use_This')}}
                                        </b-button>
                                      </template>
                                      <template v-else>
                                        <i class="i-Yes" style="font-size: 20px;"></i>
                                      </template>
                                    </td>
                                  </tr>
                                </tbody>
...
export default {
  ...
  data() {
    return {
      ...
      displaySavedPaymentMethodsByLine: {},
      displayFormNewCard: false,
      selectedCard: null, // kept for backward compatibility
      card_id: null, // default source id
      selectedCardByLine: {},
      is_new_credit_card: false,
      hasSavedPaymentMethod: false,
      useSavedPaymentMethod: false,
      submit_showing_credit_card: false,
      submit_showing_credit_card_by_line: {},
      localSavedPaymentMethods: [],
  ...
  computed: {
    ...
    selectedCardsByLineIds() {
      const result = {};
      const map = this.selectedCardByLine || {};
      Object.keys(map).forEach(k => {
        const c = map[k];
        if (c && c.card_id) result[k] = c.card_id;
      });
      return result;
    }
    ,
  ...
  methods: {
    isSavedMode(lineIndex) {
      const map = this.displaySavedPaymentMethodsByLine || {};
      return !!map[lineIndex];
    },
    isLineLoadingSavedCards(lineIndex) {
      const map = this.submit_showing_credit_card_by_line || {};
      return !!map[lineIndex];
    },
    hasAnyNewCardToTokenize() {
      return (this.paymentLines || []).some((line, idx) => this.isLineCreditCard(line) && !(this.selectedCardByLine && this.selectedCardByLine[idx]));
    },
    getDefaultSavedCard() {
      const id = this.card_id;
      if (!id) return null;
      const list = Array.isArray(this.localSavedPaymentMethods) ? this.localSavedPaymentMethods : [];
      return list.find(c => c && c.card_id && String(c.card_id) === String(id)) || null;
    },
    formatCardExpiry(card) {
      const pre = card && card.exp;
      if (pre && String(pre).trim()) return String(pre).trim();
      const m = card && (card.exp_month || card.card_exp_month);
      const y = card && (card.exp_year || card.card_exp_year);
      const month = Number(m);
      const year = Number(y);
      if (!Number.isFinite(month) || month < 1 || month > 12 || !Number.isFinite(year)) return '—';
      const mm = month < 10 ? `0${month}` : String(month);
      const yyyy = String(year);
      const yy = yyyy.length === 2 ? yyyy : yyyy.slice(-2);
      return `${mm}/${yy}`;
    },
    ...
    changePaymentMethod(line, method) {
      ...
      // If credit card selected and Stripe configured, trigger saved/new card flow
      if (this.isCreditCardMethod(method)) {
        // Set active line context for per-line UI and defaulting
    ...
    addPaymentLine() {
      ...
      // Keep existing selections intact; do not auto-override selectedCardByLine for previous lines
      // Initialize entry for new line as undefined (will be set when user selects a card or when default applies)
      if (!this.selectedCardByLine) this.selectedCardByLine = {};
      if (!this.displaySavedPaymentMethodsByLine) this.displaySavedPaymentMethodsByLine = {};
      // no-op for new line preselection here; handled in onCreditCardSelected only when empty
    },
    removePaymentLine(index) {
      ...
      // Reindex per-line mappings to prevent state leaking across lines
      this.reindexLineStateAfterRemoval(index);
    },
    ...
    show_new_credit_card(lineIndex) {
      this.displaySavedPaymentMethods = false;
      if (!this.displaySavedPaymentMethodsByLine) this.displaySavedPaymentMethodsByLine = {};
      // set active line immediately
      this.activeCardLineIndex = typeof lineIndex === 'number' ? lineIndex : 0;
      // mark this line as new card mode
      this.$set ? this.$set(this.displaySavedPaymentMethodsByLine, this.activeCardLineIndex, false) : (this.displaySavedPaymentMethodsByLine[this.activeCardLineIndex] = false);
      this.displayFormNewCard = true;
      this.is_new_credit_card = true;
      this.isCardInputComplete = false;
      // ensure any previous element is removed before remounting
      try { this.destroyCardElementForLine(this.activeCardLineIndex); } catch (e) { /* ignore */ }
      // Clear saved card selection for this line when switching to new card entry
      if (this.selectedCardByLine && Object.prototype.hasOwnProperty.call(this.selectedCardByLine, this.activeCardLineIndex)) {
        try { this.$delete ? this.$delete(this.selectedCardByLine, this.activeCardLineIndex) : delete this.selectedCardByLine[this.activeCardLineIndex]; } catch (e) { /* ignore */ }
      }
      this.$nextTick(() => setTimeout(() => this.loadStripePayment(this.activeCardLineIndex), 0));
    },
    show_saved_credit_card(lineIndex) {
      this.displaySavedPaymentMethods = true;
      if (!this.displaySavedPaymentMethodsByLine) this.displaySavedPaymentMethodsByLine = {};
      this.$set ? this.$set(this.displaySavedPaymentMethodsByLine, lineIndex, true) : (this.displaySavedPaymentMethodsByLine[lineIndex] = true);
      this.displayFormNewCard = false;
      this.is_new_credit_card = false;
      this.isCardInputComplete = false;
      this.activeCardLineIndex = typeof lineIndex === 'number' ? lineIndex : 0;
      // Unmount any per-line Stripe element when switching back to saved card for that line
      this.destroyCardElementForLine(this.activeCardLineIndex);
      // If no explicit selection for this line, preselect default card only for this line
      if (!this.selectedCardByLine || !this.selectedCardByLine[this.activeCardLineIndex]) {
        const def = this.getDefaultSavedCard();
        if (def) {
          this.$set ? this.$set(this.selectedCardByLine, this.activeCardLineIndex, def) : (this.selectedCardByLine[this.activeCardLineIndex] = def);
        }
      }
    },
    isSelectedCard(card) {
      if (!card) return false;
      if (this.selectedCard && this.selectedCard === card) return true;
      return (this.card_id && card.card_id && String(this.card_id) === String(card.card_id));
    },
    isSelectedCardForLine(card, lineIndex) {
      if (!card) return false;
      const entry = this.selectedCardByLine && this.selectedCardByLine[lineIndex];
      if (entry && entry.card_id && card.card_id) return String(entry.card_id) === String(card.card_id);
      // If no explicit selection, fall back to default card id
      if (!entry) {
        const def = this.getDefaultSavedCard();
        if (def && def.card_id && card.card_id) return String(def.card_id) === String(card.card_id);
      }
      return false;
    },
    selectCard(card) {
      this.selectedCard = card;
      this.card_id = card && card.card_id ? card.card_id : null;
    },
    selectCardForLine(card, lineIndex) {
      if (!this.selectedCardByLine) this.selectedCardByLine = {};
      this.$set ? this.$set(this.selectedCardByLine, lineIndex, card) : (this.selectedCardByLine[lineIndex] = card);
    },
    async onCreditCardSelected() {
      try {
        if (!this.clientId) {
          // No client to fetch saved cards for
          this.hasSavedPaymentMethod = false;
          this.useSavedPaymentMethod = false;
          this.show_new_credit_card(this.activeCardLineIndex || 0);
          return;
        }
        // mark only the active line as loading
        if (!this.submit_showing_credit_card_by_line) this.submit_showing_credit_card_by_line = {};
        this.$set ? this.$set(this.submit_showing_credit_card_by_line, this.activeCardLineIndex, true) : (this.submit_showing_credit_card_by_line[this.activeCardLineIndex] = true);
        this.submit_showing_credit_card = true;
        this.selectedCard = null;
        this.card_id = '';
        // Fetch saved payment methods for this client
        const response = await axios.get(`/retrieve-customer?customerId=${this.clientId}`);
        this.localSavedPaymentMethods = (response && response.data && response.data.data) ? response.data.data : [];
        this.card_id = (response && response.data && response.data.customer_default_source) ? response.data.customer_default_source : '';
        this.hasSavedPaymentMethod = Array.isArray(this.localSavedPaymentMethods) && this.localSavedPaymentMethods.length > 0;
        this.useSavedPaymentMethod = this.hasSavedPaymentMethod;
        this.is_new_credit_card = !this.hasSavedPaymentMethod;
        // Only affect the active line to avoid leaking selection to other lines
        if (this.hasSavedPaymentMethod) {
          this.show_saved_credit_card(this.activeCardLineIndex || 0);
        } else {
          this.show_new_credit_card(this.activeCardLineIndex || 0);
        }
      } catch (e) {
        // Fall back to new card form
        this.hasSavedPaymentMethod = false;
        this.useSavedPaymentMethod = false;
        this.is_new_credit_card = true;
        this.card_id = '';
        ...
        // clear loading for the active line only
        if (this.submit_showing_credit_card_by_line && Object.prototype.hasOwnProperty.call(this.submit_showing_credit_card_by_line, this.activeCardLineIndex)) {
          try { this.$set ? this.$set(this.submit_showing_credit_card_by_line, this.activeCardLineIndex, false) : (this.submit_showing_credit_card_by_line[this.activeCardLineIndex] = false); } catch (e) { /* ignore */ }
        }
    ...
    async loadStripePayment(lineIndex) {
      try {
        ...
        const mountSelector = `#${containerId}`;
    ...
    reindexLineStateAfterRemoval(removedIndex) {
      // Rebuild selectedCardByLine with shifted indices
      const oldSel = this.selectedCardByLine || {};
      const newSel = {};
      Object.keys(oldSel).forEach(k => {
        const i = Number(k);
        if (Number.isNaN(i)) return;
        if (i < removedIndex) newSel[i] = oldSel[i];
        else if (i > removedIndex) newSel[i - 1] = oldSel[i];
      });
      this.selectedCardByLine = newSel;

      // Rebuild saved-mode map with shifted indices
      const oldSaved = this.displaySavedPaymentMethodsByLine || {};
      const newSaved = {};
      Object.keys(oldSaved).forEach(k => {
        const i = Number(k);
        if (Number.isNaN(i)) return;
        if (i < removedIndex) newSaved[i] = oldSaved[i];
        else if (i > removedIndex) newSaved[i - 1] = oldSaved[i];
      });
      this.displaySavedPaymentMethodsByLine = newSaved;

      // Rebuild per-line loading map
      const oldLoading = this.submit_showing_credit_card_by_line || {};
      const newLoading = {};
      Object.keys(oldLoading).forEach(k => {
        const i = Number(k);
        if (Number.isNaN(i)) return;
        if (i < removedIndex) newLoading[i] = oldLoading[i];
        else if (i > removedIndex) newLoading[i - 1] = oldLoading[i];
      });
      this.submit_showing_credit_card_by_line = newLoading;

      // Adjust active index
      if (this.activeCardLineIndex > removedIndex) {
        this.activeCardLineIndex = this.activeCardLineIndex - 1;
      } else if (this.activeCardLineIndex === removedIndex) {
    ...
    async processPayment() {
      ...
      try {
        ...
        const newCardIndices = (this.paymentLines || []).map((line, i) => {
          const needsNew = this.isLineCreditCard(line) && !(this.selectedCardByLine && this.selectedCardByLine[i]);
        ...
        const payload = {
          ...
          payments: (this.paymentLines || []).map((l, idx) => ({
            amount: Number(l.amount) || 0,
            payment_method_id: l.paymentMethodId,
            account_id: l.accountId || this.paymentForm.accountId || null,
            selected_card_id: this.selectedCardsByLineIds && this.selectedCardsByLineIds[idx] ? this.selectedCardsByLineIds[idx] : null
          })),
          ...
          selected_cards_by_line: this.selectedCardsByLineIds,
    ...
    async CreatePOS() {
      ...
      if (multi && total > due) {
        this.makeToast('warning', this.$t ? this.$t('TotalPaidExceedsGrandTotalForMultiPayment') : 'Total paid exceeds grand total for multiple payments', this.$t ? this.$t('Warning') : 'Warning');
        if (typeof NProgress !== 'undefined') NProgress.done();
        return;
      }
      ...
      if (anyNewCard) {
        ...
        const needsNewCard = Object.keys(this.cardElementsByLine || {}).some(i => {
          ...
          const usingSaved = this.selectedCardByLine && this.selectedCardByLine[li];
          return this.isLineCreditCard(line) && !usingSaved;
        });
        if (!needsNewCard) {
          this.makeToast('danger', this.$t ? this.$t('Please_enter_credit_card_details') : 'Please enter credit card details', this.$t ? this.$t('Warning') : 'Warning');
          if (typeof NProgress !== 'undefined') NProgress.done();
    ...
    resetForm() {
      ...
      this.paymentLines = [];
      this.selectedCardByLine = {};
      this.displaySavedPaymentMethodsByLine = {};
      this.submit_showing_credit_card_by_line = {};
