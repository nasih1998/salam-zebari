...
class QuickBooksService
{
    ...
    private function qboEscape(string $s): string { return str_replace("'", "''", $s); }
    private function qboEscapeLike(string $s): string { return str_replace(['%','_'], ['\\%','\\_'], $s); }
    ...
    private function ensureCustomer(Client $client, DataService $ds): array
    {
        if (!empty($client->quickbooks_id)) {
            ...
            if (is_array($hit) && isset($hit[0]->Id)) return ['ok'=>true,'id'=>(string)$hit[0]->Id];
            $client->quickbooks_id = null; $client->save();
        }
        ...
        if (!empty($client->email)) {
            ...
            if (is_array($hit) && isset($hit[0]->Id)) {
                $client->quickbooks_id = (string)$hit[0]->Id; $client->save();
        ...
        if (!empty($client->email)) $payload['PrimaryEmailAddr'] = ['Address' => $client->email];
        if (!empty($client->phone ?? null)) $payload['PrimaryPhone'] = ['FreeFormNumber' => (string)$client->phone];

        $res = $ds->Add(Customer::create($payload));
        if ($e = $this->dsError($ds)) return ['ok'=>false, 'error'=>'QBO Create Customer failed'] + $e;

        $client->quickbooks_id = (string)($res->Id ?? null); $client->save();
        return ['ok'=>true,'id'=>$client->quickbooks_id];
    }
    ...
    private function ensureItem(Product $product, DataService $ds): array
    {
        ...
        if (!empty($product->code ?? null)) $payload['Sku'] = (string)$product->code;

        $res = $ds->Add(Item::create($payload));
        if ($e = $this->dsError($ds)) return ['ok'=>false, 'error'=>'QBO Create Item failed'] + $e;

        $product->quickbooks_id = (string)($res->Id ?? null); $product->save();
        return ['ok'=>true,'id'=>$product->quickbooks_id];
    }
    ...
    private function buildInvoiceLines(Sale $sale, DataService $ds): array
    {
        ...
        foreach ($sale->saleDetails as $d) {
            $p = $d->product;
            if (!$p) return ['ok'=>false,'error'=>"Missing product id {$d->product_id} on detail {$d->id}"];
            $map = $this->ensureItem($p, $ds);
            if (!($map['ok'] ?? false)) return ['ok'=>false] + $map;
    ...
    public function createInvoice(Sale $sale, ?string $realmId = null, ?string $environment = null): array
    {
        ...
        if (!empty($sale->Ref)) $payload['DocNumber'] = (string)$sale->Ref;
