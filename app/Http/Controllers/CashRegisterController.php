...
class CashRegisterController extends BaseController
{
    ...
    public function report(Request $request)
    {
        ...
        if (!in_array($order, $allowedSorts, true)) { $order = 'opened_at'; }
        ...
        if ($request->filled('from'))         { $query->whereDate('opened_at', '>=', $request->from); }
        if ($request->filled('to'))           { $query->whereDate('opened_at', '<=', $request->to); }
