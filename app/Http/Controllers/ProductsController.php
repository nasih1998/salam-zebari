    ...
    public function index(Request $request)
    {
        ...
        foreach ($products as $product) {
            ...
            if ($product->type === 'is_single') {
                ...
                $item['cost']  = $helpers->formatNumber($product->cost);
                $item['price'] = $helpers->formatNumber($product->price);
                $item['wholesale_price'] = $helpers->formatNumber($product->wholesale_price ?? 0);
                ...
                $item['type']  = 'Combo';
                $item['name']  = $product->name; // PLAIN TEXT
                $item['cost']  = rtrim(rtrim(number_format((float) $product->cost, 2, '.', ','), '0'), '.');
                $item['price'] = rtrim(rtrim(number_format((float) $product->price, 2, '.', ','), '0'), '.');
                $item['wholesale_price'] = rtrim(rtrim(number_format((float) ($product->wholesale_price ?? 0), 2, '.', ','), '0'), '.');
                $item['unit']  = optional($product->unit)->ShortName;
                ...
                $item['cost']  = $variants->map(fn ($v) => $helpers->formatNumber($v->cost))->implode("\n");
                $item['price'] = $variants->map(fn ($v) => $helpers->formatNumber($v->price))->implode("\n");
                $item['wholesale_price'] = $variants->map(fn ($v) => $helpers->formatNumber($v->wholesale ?? 0))->implode("\n");
                $item['unit']  = optional($product->unit)->ShortName;
